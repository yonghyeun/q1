...
class WorktreeIssueMetadataTests(unittest.TestCase):
    ...
    def run_script(self, cwd: Path, *args: str) -> subprocess.CompletedProcess[str]:
        return subprocess.run(
            ["bash", "./scripts/repo/worktree_issue_metadata.sh", *args],
            cwd=cwd,
            text=True,
            capture_output=True,
            check=False,
    ...
    def test_write_persists_issue_metadata(self) -> None:
        ...
        self.assertIn("q1.issue.number=19", read_result.stdout)
        self.assertIn("q1.issue.title=issue title", read_result.stdout)
        self.assertIn("q1.issue.recordedBy=task_start", read_result.stdout)
    ...
    def test_clear_removes_issue_metadata(self) -> None:
        _root, worktree = self.make_repo()
