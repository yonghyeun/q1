...
- Date: 2026-03-04
- Context: 이 프로젝트는 WBS를 task 단위로 쪼개고(병렬), task마다 에이전트를 붙여 구현한 뒤 AC를 하네스로 검증하는 방식을 목표로 한다. 병렬 개발에서는 작은 용어/형식 불일치가 통합(merge) 시 큰 재작업으로 커진다. `glossary.md`는 "개념의 이름(유비쿼터스 언어)"을 정렬해주지만, 타입/응답형식/이벤트 스키마/저장 포맷 같은 "접점(interface)"까지 강제해주지는 못한다. 또한 YouTube 연동은 서버 측 integration boundary를 거칠 가능성이 높고, 이때의 입력/응답 형식도 프론트-통합 경계 계약이 된다.
- Decision: 병렬 task 개발의 통합 비용을 낮추기 위해, "contracts"를 cross-task 접점의 Source of Truth(SoT)로 둔다.
  - contracts에 포함되는 것(예): Domain 타입(`ChannelSource`, `Video`, `Note`, `Marker`), YouTube integration request/response, Analytics 이벤트 스키마(`open_method`, `feed_filter` 등), LocalStorage 저장 스키마/키 네이밍.
