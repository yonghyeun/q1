...
- 해당 영상의 Player + Note write 화면으로 바로 진입한다(노트는 `video_id` 기준으로 저장/재호출).

## Core Flow (Revisit Path)

1. 유저가 `/notes`에서 작성한 note가 있는 영상을 본다.
2. note를 다시 볼 영상을 선택한다.
3. 해당 영상의 Player + Note read 화면으로 진입한다.
4. 필요하면 같은 `video_id`를 유지한 채 write mode로 전환한다.
...
- Video Feed 화면
- Note List 화면
- Player + Note 화면(write/read mode, 핵심)
...
- `VideoOpen`: `video_id` + `open_method` (`feed|url|note_list`) + `opened_at` (선택: 최근 열어본 영상)
